...
<body>
    ...
    <div class="container">
        <div class="header">
            <h2>मन्त्र क्यान्टिन</h2>
            <p>व्यवहार व्यवस्थापन प्रणाली</p>
        </div>
        ...
        <div class="filter-section">
            <form method="GET" action="{{ route('transactions.index') }}" class="filter-switch">
                ...
                <div class="filter-group">
                    <label for="year"><i class="fas fa-calendar-alt"></i> वर्ष:</label>
                    <select name="year_filter" id="year" onchange="this.form.submit()">
                        <option value="all">वर्ष</option>
                        @isset($years)
                ...
                <div class="filter-group">
                    <label for="month"><i class="fas fa-calendar"></i> महिना:</label>
                    <select name="month_filter" id="month" onchange="this.form.submit()">
                        <option value="all">महिना</option>
                        @isset($months)
                ...
                <div class="filter-group">
                    <label for="date"><i class="fas fa-calendar-day"></i> मिति:</label>
                    <select name="date_filter" id="date" onchange="this.form.submit()">
                        <option value="all">मिति</option>
                ...
                @isset($transactions)
                    @forelse ($transactions as $index => $transaction)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>
                                {{ app()->getLocale() == 'en' ? $transaction->date : \Carbon\Carbon::parse($transaction->date)->locale('ne')->translatedFormat('Y-m-d') }}
                            </td>
                            <td>
                                {{ app()->getLocale() == 'en' ? $transaction->day : \Carbon\Carbon::parse($transaction->day)->locale('ne')->dayName }}
                            </td>
                            <td>{{ number_format($transaction->amount, 2) }}</td>
                            <td>
                                <button class="remove-btn" onclick="removeTransaction(this)">
                                    <i class="fas fa-trash"></i> हटाउनुहोस्
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="no-data">कुनै डाटा फेला परेन।</td></tr>
                    @endforelse
                @else
                    <tr><td colspan="5" class="no-data">Transactions data not loaded</td></tr>
                @endisset
            </tbody>
        </table>
    ...
    <script>
        ...
        function checkNoData() {
            ...
            if (!hasData && !tbody.querySelector('.no-data')) {
                tbody.innerHTML = '<tr><td colspan="5" class="no-data">कुनै डाटा फेला परेन।</td></tr>';
            }
        }
